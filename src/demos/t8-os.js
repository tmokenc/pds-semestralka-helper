...
function initPacketRate() {
  const calc = () => {
    ...
    document.getElementById('rate-out').innerHTML =
      ...
      `<span style="font-size:12px;color:var(--text-muted)">Proto se používá batching, kernel bypass (DPDK), in-kernel akcelerace (XDP).</span>`;
  };
  document.getElementById('rate-calc').addEventListener('click', calc);
  calc();
}
...
function initNetfilter() {
  ...
  function draw() {
    ...
    pipe.hooks.forEach((h, i) => {
      ...
      const fill = isActive ? '#0066cc' : isDone ? '#b8d4ff' : isHook ? '#fce5ff' : '#fff';
      svgEl('rect', { class: `hook-box ${isActive ? 'active' : ''}`, x, y: 90, width: stepW - 16, height: 60, fill, stroke: isActive ? '#003d7a' : '#7a3ea1', 'stroke-width': isActive ? 3 : 1.5, rx: 6 }, svg);
      svgEl('text', { x: x + (stepW - 16) / 2, y: 120, 'text-anchor': 'middle', 'font-weight': 600, 'font-size': 12, 'font-family': 'monospace', fill: isActive ? 'white' : '#1a1a1a', text: h }, svg);
      svgEl('text', { x: x + (stepW - 16) / 2, y: 138, 'text-anchor': 'middle', 'font-size': 10, fill: isActive ? 'white' : '#666', text: isHook ? 'netfilter hook' : h === 'routing' ? 'FIB lookup' : 'aplikace' }, svg);
      if (i < pipe.hooks.length - 1) {
        const ax = x + stepW - 14, ay = 120;
        svgEl('line', { x1: ax, y1: ay, x2: ax + 14, y2: ay, stroke: '#0066cc', 'stroke-width': 2, 'marker-end': 'url(#nf-arr)' }, svg);
      }
    });
    ...
    if (activeIdx >= 0 && activeIdx < pipe.hooks.length) {
      svgEl('text', { x: W / 2, y: 70, 'text-anchor': 'middle', 'font-size': 12, fill: '#7a3ea1', text: `Aktuálně: ${pipe.hooks[activeIdx]}` }, svg);
    }
    ...
    if (activeIdx >= 0 && tableMap[pipe.hooks[activeIdx]]) {
      ...
      document.getElementById('nf-info').innerHTML = '<strong style="color:var(--num)">✓ Paket doručen (nebo odeslán na drát).</strong>';
    } else {
...
function initAcc() {
  ...
  function draw() {
    ...
    if (userspaceFrom > 0) {
      ...
      svgEl('rect', { x: 30, y: 60, width: splitX - 30, height: 140, fill: '#fff0f0', stroke: 'none' }, svg);
      svgEl('text', { x: (30 + splitX) / 2, y: 50, 'text-anchor': 'middle', 'font-size': 11, fill: '#c73a1f', text: 'KERNEL SPACE' }, svg);
      svgEl('rect', { x: splitX, y: 60, width: W - 30 - splitX, height: 140, fill: '#f0fff5', stroke: 'none' }, svg);
      svgEl('text', { x: (splitX + W - 30) / 2, y: 50, 'text-anchor': 'middle', 'font-size': 11, fill: '#0a7a3d', text: 'USER SPACE' }, svg);
    }

    t.flow.forEach((step, i) => {
      const x = 30 + i * stepW + 6;
      const isBypass = t.bypass && (step.includes('app') || step.includes('PMD') || step.includes('User'));
      const fill = isBypass ? '#0a7a3d' : '#b8d4ff';
      svgEl('rect', { x, y: 100, width: stepW - 12, height: 50, fill, stroke: '#0066cc', 'stroke-width': 1.5, rx: 4 }, svg);
      svgEl('text', { x: x + (stepW - 12) / 2, y: 124, 'text-anchor': 'middle', 'font-size': 11, 'font-weight': 600, fill: isBypass ? 'white' : '#1a1a1a', text: step }, svg);
      if (i < t.flow.length - 1) {
        const ax = x + stepW - 10;
        svgEl('line', { x1: ax, y1: 125, x2: ax + 10, y2: 125, stroke: '#0066cc', 'stroke-width': 2, 'marker-end': 'url(#acc-arr)' }, svg);
      }
    });

    svgEl('text', { x: W / 2, y: 220, 'text-anchor': 'middle', 'font-size': 12, fill: '#666', text: `Max. propustnost: ${t.maxRate}, latence: ${t.latency}, kernel bypass: ${t.bypass ? 'ANO' : 'NE'}` }, svg);

    document.getElementById('acc-info').innerHTML = `<strong>${tech}</strong><br>${t.notes}`;
  }
  document.getElementById('acc-show').addEventListener('click', draw);
...
function initSKB() {
  ...
  function draw() {
    ...
    const W = 700, H = 240;
    // SKB structure box
    svgEl('rect', { x: 30, y: 20, width: 160, height: 200, fill: '#e8efff', stroke: '#0066cc', 'stroke-width': 2, rx: 6 }, svg);
    ...
    svgEl('rect', { x: bufX, y: bufY, width: bufW, height: bufH, fill: 'white', stroke: '#aaa', 'stroke-width': 2 }, svg);
    svgEl('text', { x: bufX + bufW / 2, y: 30, 'text-anchor': 'middle', 'font-size': 12, fill: '#666', text: 'Packet data storage' }, svg);
    // Headroom
    const headroomW = 30;
    svgEl('rect', { x: bufX, y: bufY, width: headroomW, height: bufH, fill: '#f0f0f0' }, svg);
    svgEl('text', { x: bufX + headroomW / 2, y: bufY + bufH / 2, 'text-anchor': 'middle', 'font-size': 9, fill: '#888', transform: `rotate(-90 ${bufX + headroomW / 2} ${bufY + bufH / 2})`, text: 'headroom' }, svg);

    // Headers
    const palette = ['#7a3ea1', '#0066cc', '#cc8f00', '#0a7a3d'];
    let x = bufX + headroomW;
    const dataW = bufW - headroomW - 30;
    const slotW = dataW / 4;
    headers.forEach((h, i) => {
      const isPayload = h === 'payload';
      const fill = isPayload ? '#f0fff5' : palette[(headers.length - 1 - i) % palette.length];
      svgEl('rect', { x, y: bufY, width: slotW, height: bufH, fill, stroke: '#555', 'stroke-width': 1.5 }, svg);
      svgEl('text', { x: x + slotW / 2, y: bufY + bufH / 2 + 4, 'text-anchor': 'middle', 'font-size': 12, 'font-weight': 600, fill: isPayload ? '#1a1a1a' : 'white', text: h }, svg);
      x += slotW;
    });
    // Tailroom
    svgEl('rect', { x: x, y: bufY, width: bufX + bufW - x, height: bufH, fill: '#f0f0f0' }, svg);

    // Pointers
    svgEl('line', { x1: 190, y1: 75, x2: bufX - 2, y2: bufY + 5, stroke: '#0066cc', 'stroke-width': 1.5 }, svg);
    svgEl('text', { x: bufX - 8, y: bufY + 2, 'text-anchor': 'end', 'font-size': 10, fill: '#0066cc', text: 'head' }, svg);

    svgEl('line', { x1: 190, y1: 95, x2: bufX + headroomW - 2, y2: bufY + 60, stroke: '#0066cc', 'stroke-width': 1.5 }, svg);
    svgEl('text', { x: bufX + headroomW - 4, y: bufY + 55, 'text-anchor': 'end', 'font-size': 10, fill: '#0066cc', 'font-weight': 600, text: 'data ↓' }, svg);

    svgEl('line', { x1: 190, y1: 115, x2: x + 2, y2: bufY + 80, stroke: '#0066cc', 'stroke-width': 1.5 }, svg);
    svgEl('text', { x: x + 5, y: bufY + 75, 'font-size': 10, fill: '#0066cc', 'font-weight': 600, text: '↓ tail' }, svg);

    svgEl('line', { x1: 190, y1: 135, x2: bufX + bufW + 2, y2: bufY + 115, stroke: '#0066cc', 'stroke-width': 1.5 }, svg);
    svgEl('text', { x: bufX + bufW + 5, y: bufY + 110, 'font-size': 10, fill: '#0066cc', text: 'end' }, svg);
...
function initRSS() {
  ...
  function go() {
    ...
    svgEl('text', { x: 30, y: 50, 'font-size': 12, fill: '#666', text: `hash % ${q} = ${queue}` }, svg);

    const qW = (W - 60) / q;
    for (let i = 0; i < q; i++) {
      const isSel = i === queue;
      svgEl('rect', { x: 30 + i * qW, y: 80, width: qW - 4, height: 60, fill: isSel ? '#0066cc' : '#fff', stroke: isSel ? '#003d7a' : '#888', 'stroke-width': isSel ? 3 : 1.5, rx: 4 }, svg);
      svgEl('text', { x: 30 + i * qW + (qW - 4) / 2, y: 110, 'text-anchor': 'middle', 'font-size': 11, 'font-weight': 600, fill: isSel ? 'white' : '#444', text: `Queue ${i}` }, svg);
      svgEl('text', { x: 30 + i * qW + (qW - 4) / 2, y: 128, 'text-anchor': 'middle', 'font-size': 10, fill: isSel ? 'white' : '#888', text: `CPU ${i}` }, svg);
    }
    svgEl('text', { x: 350, y: 175, 'text-anchor': 'middle', 'font-size': 12, fill: '#7a3ea1', text: 'Stejný flow → vždy stejná queue → stejný CPU (no reorder)' }, svg);
    svgEl('text', { x: 350, y: 195, 'text-anchor': 'middle', 'font-size': 11, fill: '#c73a1f', text: 'Tunelování (GRE, IPSec) má stejnou vnější hlavičku → všechen traffic v jedné queue!' }, svg);
  ...
  document.getElementById('rss-go').addEventListener('click', go);
...
function initXDP() {
  const SCENARIOS = {
    ddos: { action: 'XDP_DROP', why: 'SYN flood — zahodit hned v driveru, žádný sk_buff alloc, žádný kernel stack overhead. Cloudflare drop ~50 Mpps per CPU jádro.', color: '#c73a1f' },
    lb: { action: 'XDP_TX', why: 'L4 load balancer (Katran/Facebook): přepiš dst IP/MAC na backend a pošli zpět ven. Žádný kernel routing, ~10× rychlejší než iptables.', color: '#7a3ea1' },
    monitor: { action: 'XDP_PASS', why: 'Pouhý monitoring/IDS — neměnit, kopírovat do user space přes perf buffer, paket pokračuje normálním stackem.', color: '#0a7a3d' },
    ping: { action: 'XDP_TX', why: 'Echo reply přímo z driveru — přepiš src/dst a pošli zpět. Pro testing nebo „heartbeat odpovědi" bez zatížení kernelu.', color: '#cc8f00' },
    route: { action: 'XDP_REDIRECT', why: 'Bridge mezi 2 NIC bez kernel routingu. AF_XDP socket dostane paket nebo se pošle na druhý interface přímo.', color: '#0066cc' },
  };
  function go() {
    ...
    clearSvg(svg);
    arrowDef(svg, 'xdp-arr', '#0066cc');
    const W = 700;
    svgEl('rect', { x: 30, y: 40, width: 130, height: 60, fill: '#b8d4ff', stroke: '#0066cc', 'stroke-width': 2, rx: 6 }, svg);
    svgEl('text', { x: 95, y: 75, 'text-anchor': 'middle', 'font-weight': 600, text: 'NIC (driver)' }, svg);
    svgEl('rect', { x: 200, y: 40, width: 160, height: 60, fill: '#fce5ff', stroke: '#7a3ea1', 'stroke-width': 2, rx: 6 }, svg);
    svgEl('text', { x: 280, y: 65, 'text-anchor': 'middle', 'font-weight': 600, text: 'eBPF/XDP program' }, svg);
    svgEl('text', { x: 280, y: 82, 'text-anchor': 'middle', 'font-size': 11, fill: '#666', text: 'rozhodnutí ↓' }, svg);
    // Arrow
    svgEl('line', { x1: 160, y1: 70, x2: 198, y2: 70, stroke: '#0066cc', 'stroke-width': 2, 'marker-end': 'url(#xdp-arr)' }, svg);

    // Actions
    const actions = [
      { name: 'DROP', y: 150, color: '#c73a1f', desc: 'zahodit' },
      { name: 'PASS', y: 150, color: '#0a7a3d', desc: 'do kernel stacku', x: 420 },
      { name: 'REDIRECT', y: 220, color: '#0066cc', desc: 'na jiný interface' },
      { name: 'TX', y: 220, color: '#cc8f00', desc: 'zpět odesílateli', x: 420 },
    ];
    let pos = 0;
    actions.forEach((a, i) => {
      const x = a.x || 70 + (i % 2) * 130;
      const isSelected = s.action.includes(a.name);
      svgEl('rect', { x, y: a.y, width: 140, height: 50, fill: isSelected ? a.color : '#fff', stroke: a.color, 'stroke-width': isSelected ? 3 : 2, rx: 6 }, svg);
      ...
      svgEl('text', { x: x + 70, y: a.y + 40, 'text-anchor': 'middle', 'font-size': 11, fill: isSelected ? 'white' : '#666', text: a.desc }, svg);
      // Arrow from program
      if (isSelected) {
  ...
  document.getElementById('xdp-go').addEventListener('click', go);
...
function initIptables() {
  function go() {
    ...
    document.getElementById('ipt-out').innerHTML =
      ...
      `<tr><td><strong>Netfilter hook</strong></td><td class="mono" style="color:#0066cc"><strong>${hook}</strong></td><td>${hook === '?' ? '' : 'zde se pravidlo vyhodnotí'}</td></tr>` +
      ...
      `</table>` +
      `<div style="margin-top:8px;font-size:12px;color:var(--text-muted)">Příklad pochopení: <code>POSTROUTING + MASQUERADE</code> = NAT na výstupu pomocí adresy výstupního rozhraní (klasické sdílení internetu).</div>`;
  }
  document.getElementById('ipt-go').addEventListener('click', go);
  go();
}
